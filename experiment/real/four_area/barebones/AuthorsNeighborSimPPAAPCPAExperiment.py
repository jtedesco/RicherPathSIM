...
class AuthorsNeighborSimPPAAPCPAExperiment(Experiment):
    """
    ...
    def runFor(self, author, citationCounts, publicationCounts, weights):
        ...
        appaPath = os.path.join('results', 'authors', 'intermediate', '%s-neighborsim-ppa' % author.replace(' ', ''))
...
def run(citationCounts, publicationCounts, weights = (0.5, 0.5)):
    experiment = AuthorsNeighborSimPPAAPCPAExperiment(
        None,
        'Most Similar PPA-APCPA NeighborSim Authors',
        outputFilePath = os.path.join('results','authors','ppa-apcpaNeighborSim-%1.1f-%1.1f' % weights)
    )
